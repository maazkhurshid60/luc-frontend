                ...
                <div class="back-link">
                    ...
                    <li class="page-item">
                        <a class="page-link back-button secondary-btn d-flex justify-content-center align-items-center pg-res-btn {{ !$paginator->previousPageUrl() ? 'disabled' : '' }}"
                            onclick="location.href='{{ $paginator->previousPageUrl() }}{{ isset($selected_category) ? '&category_id=' . $selected_category : '' }}'"
                            aria-label="Back">
                    ...
                    @for ($i = 1; $i <= $paginator->lastPage(); $i++)
                        <li class="page-item">
                            <a class="page-link pg-items body-txt1 secondary--clr pg-item{{ $i == $paginator->currentPage() ? '-active' : 's' }}"
                                href="{{ $paginator->url($i) }}{{ isset($selected_category) ? '&category_id=' . $selected_category : '' }}">{{ $i }}</a>
                        </li>
                    @endfor
                ...
                <div class="next-link">
                    ...
                    <li class="page-item">
                        <a class="page-link next-button secondary-btn d-flex justify-content-center align-items-center pg-res-btn {{ !$paginator->nextPageUrl() ? 'disabled' : '' }}"
                            onclick="location.href='{{ $paginator->nextPageUrl() }}{{ isset($selected_category) ? '&category_id=' . $selected_category : '' }}'"
                            aria-label="Next">
                            <p class="body-txt1 d-md-block d-none mb-0">
                                Next
                            </p>
