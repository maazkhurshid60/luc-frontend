...
@section('content')
    @php
        $columns = ['ID', 'name', 'email', 'contact No.', 'date'];
        $data_route = route('feedback.datatable');
        $update_url = route('feedback.show', 11);
        $destroy_url = route('feedback.destroy', 11);
        $heading = 'Feedbacks';
    @endphp
...
@section('custom-scripts')
    ...
    <script type="text/javascript">
        ...
        $('input[name=name]').keyup(oTable.draw);
        $('select[name=technology],select[name=service]').change(oTable.draw);
